 ...
 *  
 *  ARGS:
 *  - TOML_API: if defined, will be added before function declarations.
 *  - TOML_CHECK_UFT8: if defined and non-zero, will check UTF-8 encoding of strings.
 *  - TOML_MALLOC, TOML_REALLOC, TOML_FREE: if defined, will be used for memory management.
 */
...
#ifndef TOML_CHECK_UFT8
#define TOML_CHECK_UFT8 0
#endif

#ifndef TOML_MALLOC
#define TOML_MALLOC(n) malloc(n)
#endif

#ifndef TOML_REALLOC
#define TOML_REALLOC(p, n) realloc(p, n)
#endif

#ifndef TOML_FREE
#define TOML_FREE(p) free(p)
#endif
...
struct toml_datum_t {
  ...
  union {
    ...
    struct {
      ...
      int32_t len;     // length excluding the terminating NUL.
    } str;
 ...
 */
TOML_API toml_result_t toml_parse(const char *src, size_t len);
 ...
 */
TOML_API toml_result_t toml_parse_file(FILE *fp);

/**
 * Parse a toml file. Returns a toml_result which must be freed
 * using toml_free() eventually.
 */
TOML_API toml_result_t toml_parse_file_ex(const char *fname);

/**
 * Release the result.
 */
TOML_API void toml_free(toml_result_t result);

/**
* Convert a toml_type_t to a string. The returned string does not need to be freed.
*/
TOML_API const char *toml_type_to_str(toml_type_t ty);

/**
 * Find a key in a toml_table. Return the value of the key if found,
 * or a TOML_UNKNOWN otherwise.
 */
TOML_API toml_datum_t toml_get(toml_datum_t table, const char *key);
 ...
 */
TOML_API toml_datum_t toml_seek(toml_datum_t table,
 ...
 *         override
 */
TOML_API toml_result_t toml_merge(const toml_result_t *r1,
 ...
 *  sensitive.
 */
TOML_API bool toml_equiv(const toml_result_t *r1, const toml_result_t *r2);
...
static int RETERROR(ebuf_t ebuf, int lineno, const char *fmt, ...) {
  ...
  char *q = p + ebuf.len;
  if (lineno) {
    snprintf(p, (size_t)(q - p), "(line %d) ", lineno);
    p += strlen(p);
  }
  vsnprintf(p, (size_t)(q - p), fmt, args);
  return -1;
}
 ...
 */
static pool_t *pool_create(size_t N) {
  if (N <= 0) {
    N = 100; // minimum
  }
  size_t totalsz = sizeof(pool_t) + N;
  pool_t *pool = TOML_MALLOC(totalsz);
  if (!pool) {
    return NULL;
  }
 ...
 */
static void pool_destroy(pool_t *pool) { TOML_FREE(pool); }

/**
 *  Allocate n bytes from pool. Return the memory allocated on
 *  success, or NULL if out of memory.
 */
static char *pool_alloc(pool_t *pool, size_t n) {
...
static void scan_init(scanner_t *sp, const char *src, size_t len, char *errbuf,
                      size_t errbufsz);
...
static toml_datum_t *tab_emplace(toml_datum_t *tab, span_t key,
  ...
  int32_t N = tab->u.tab.size;
  for (int32_t i = 0; i < N; i++) {
    if (tab->u.tab.len[i] == key.len &&
        0 == memcmp(tab->u.tab.key[i], key.ptr, (size_t)key.len)) {
      return &tab->u.tab.value[i];
    }
  }
  // Expand pkey[], plen[] and value[]
  {
    char **pkey = TOML_REALLOC(tab->u.tab.key, sizeof(*pkey) * align8((size_t)N + 1));
    if (!pkey) {
  ...
  {
    int32_t *plen = TOML_REALLOC(tab->u.tab.len, sizeof(*plen) * align8((size_t)N + 1));
    if (!plen) {
      *reason = "out of memory";
      return NULL;
    }
  ...
  {
    toml_datum_t *value =
        TOML_REALLOC(tab->u.tab.value, sizeof(*value) * align8((size_t)N + 1));
    if (!value) {
      *reason = "out of memory";
      return NULL;
    }
...
static int32_t tab_find(toml_datum_t *tab, span_t key) {
  assert(tab->type == TOML_TABLE);
  for (int32_t i = 0, top = tab->u.tab.size; i < top; i++) {
    if (tab->u.tab.len[i] == key.len &&
        0 == memcmp(tab->u.tab.key[i], key.ptr, (size_t)key.len)) {
      return i;
    }
...
static toml_datum_t *arr_emplace(toml_datum_t *arr, const char **reason) {
  ...
  int32_t n = arr->u.arr.size;
  toml_datum_t *elem = TOML_REALLOC(arr->u.arr.elem, sizeof(*elem) * align8((size_t)n + 1));
  if (!elem) {
    *reason = "out of memory";
    return NULL;
  }
...
static void datum_free(toml_datum_t *datum) {
  if (datum->type == TOML_TABLE) {
    for (int32_t i = 0, top = datum->u.tab.size; i < top; i++) {
      datum_free(&datum->u.tab.value[i]);
    }
    TOML_FREE(datum->u.tab.key);
    TOML_FREE(datum->u.tab.len);
    TOML_FREE(datum->u.tab.value);
  } else if (datum->type == TOML_ARRAY) {
    for (int32_t i = 0, top = datum->u.arr.size; i < top; i++) {
      datum_free(&datum->u.arr.elem[i]);
    }
    TOML_FREE(datum->u.arr.elem);
  }
...
static int datum_copy(toml_datum_t *dst, toml_datum_t src, pool_t *pool,
  ...
  *dst = mkdatum(src.type);
  switch (src.type) {
  case TOML_STRING:
    dst->u.str.ptr = pool_alloc(pool, (size_t)src.u.str.len + 1);
    if (!dst->u.str.ptr) {
      *reason = "out of memory";
      goto bail;
    }
    dst->u.str.len = src.u.str.len;
    memcpy((char *)dst->u.str.ptr, src.u.str.ptr, (size_t)src.u.str.len + 1);
    break;
  case TOML_TABLE:
    for (int32_t i = 0; i < src.u.tab.size; i++) {
    ...
    break;
  case TOML_ARRAY:
    for (int32_t i = 0; i < src.u.arr.size; i++) {
...
static inline bool is_array_of_tables(toml_datum_t datum) {
  ...
  for (int32_t i = 0; ret && i < datum.u.arr.size; i++) {
...
static bool datum_equiv(toml_datum_t a, toml_datum_t b) {
  ...
  case TOML_STRING:
    return a.u.str.len == b.u.str.len &&
           0 == memcmp(a.u.str.ptr, b.u.str.ptr, (size_t)a.u.str.len);
  case TOML_INT64:
  ...
  case TOML_TABLE:
    ...
    for (int32_t i = 0; i < N; i++) {
      int32_t len = a.u.tab.len[i];
      if (len != b.u.tab.len[i]) {
        return false;
      }
      if (0 != memcmp(a.u.tab.key[i], b.u.tab.key[i], (size_t)len)) {
        return false;
      }
 ...
 *         override
 */
TOML_API toml_result_t toml_merge(const toml_result_t *r1, const toml_result_t *r2) {
  const char *reason = "";
  toml_result_t ret = {0};
  pool_t *pool = 0;
  if (!r1->ok) {
  ...
  {
    pool_t *r1pool = (pool_t *)r1->p__internal;
    pool_t *r2pool = (pool_t *)r2->p__internal;
 ...
 * or a TOML_UNKNOWN otherwise.
 */
TOML_API toml_datum_t toml_get(toml_datum_t datum, const char *key) {
  toml_datum_t ret = {0};
  if (datum.type == TOML_TABLE) {
    int32_t n = datum.u.tab.size;
 ...
 * chars.
 */
TOML_API toml_datum_t toml_seek(toml_datum_t table, const char *multipart_key) {
  if (table.type != TOML_TABLE) {
    return DATUM_ZERO;
  }

  char buf[128];
  size_t bufsz = strlen(multipart_key) + 1;
  if (bufsz >= sizeof(buf)) {
    return DATUM_ZERO;
  }
...
TOML_API void toml_free(toml_result_t result) {
  ...
  pool_destroy((pool_t *)result.p__internal);
}

static const char *type_to_str_tab[] = {
    [TOML_UNKNOWN] = "unknown",
    [TOML_STRING] = "string",
    [TOML_INT64] = "int64",
    [TOML_FP64] = "fp64",
 ...
 *  Convert a toml_type_t to a string. The returned string does not need to be freed.
 */
TOML_API const char *toml_type_to_str(toml_type_t type) {
    if (type < 0 ||
        type >= (toml_type_t)(sizeof(type_to_str_tab) / sizeof(type_to_str_tab[0]))) {
        return "invalid type";
    }
    return type_to_str_tab[type];
}
...
TOML_API toml_result_t toml_parse_file(FILE *fp) {
  ...
  while (!feof(fp)) {
    ...
    if (top == max) {
      // need to extend buf[]
      size_t tmpmax = max + max / 2 + 1000;
      if (tmpmax < max) {
        // the real max is SIZE_MAX - 1 to account for terminating NUL.
        if (max < SIZE_MAX - 1) {
          tmpmax = SIZE_MAX - 1;
        } else {
      ...
      char *tmp = TOML_REALLOC(buf, tmpmax + 1);
      if (!tmp) {
...
TOML_API toml_result_t toml_parse(const char *src, size_t len) {
  ...
  // If user insists, check that src[] is a valid utf8 string.
    ...
    for (size_t i = 0; i < len;) {
      uint32_t ch;
      int n = utf8_to_ucs(src + i, (int)(len - i), &ch);
      if (n < 0) {
        snprintf(result.errmsg, sizeof(result.errmsg),
  ...
  result.p__internal = (void *)pp->pool;
  return result;
...
static int token_to_time(parser_t *pp, token_t tok, toml_datum_t *ret) {
  ...
  *ret = mkdatum(TOML_TIME);
  ret->u.ts.hour = (int16_t)tok.u.tsval.hour;
  ret->u.ts.minute = (int16_t)tok.u.tsval.minute;
  ret->u.ts.second = (int16_t)tok.u.tsval.sec;
...
static int token_to_date(parser_t *pp, token_t tok, toml_datum_t *ret) {
  ...
  *ret = mkdatum(TOML_DATE);
  ret->u.ts.year = (int16_t)tok.u.tsval.year;
  ret->u.ts.month = (int16_t)tok.u.tsval.month;
  ret->u.ts.day = (int16_t)tok.u.tsval.day;
  return 0;
}
...
static int token_to_datetimetz(parser_t *pp, token_t tok, toml_datum_t *ret) {
  ...
  *ret = mkdatum(TOML_DATETIMETZ);
  ...
  ret->u.ts.second = (int16_t)tok.u.tsval.sec;
  ret->u.ts.usec = tok.u.tsval.usec;
  ret->u.ts.tz = (int16_t)tok.u.tsval.tz;
  return 0;
}
...
static toml_datum_t *descend_keypart(parser_t *pp, int lineno,
  ...
  for (int32_t i = 0; i < keypart->nspan; i++) {
    const char *reason;
    // Find the i-th keypart
    int32_t j = tab_find(tab, keypart->span[i]);
...
static int parse_std_table_expr(parser_t *pp, token_t tok) {
  ...
  int32_t j = tab_find(tab, lastkeypart);
  if (j < 0) {
    // If not found: add it.
    if (tab->flag & FLAG_INLINED) {
...
static int parse_array_table_expr(parser_t *pp, token_t tok) {
  ...
  const char *reason;
  int32_t idx = tab_find(tab, lastkeypart);
  if (idx == -1) {
...
static int parse_keyvalue_expr(parser_t *pp, token_t tok) {
  ...
  for (int32_t i = 0; i < keypart.nspan - 1; i++) {
    int32_t j = tab_find(tab, keypart.span[i]);
    if (j < 0) {
...
static int parse_norm(parser_t *pp, token_t tok, span_t *ret_span) {
  ...
  char *p = pool_alloc(pp->pool, (size_t)tok.str.len + 1);
  if (!p) {
    return RETERROR(pp->ebuf, tok.lineno, "out of memory");
  }

  // Copy from token string into buffer
  memcpy(p, tok.str.ptr, (size_t)tok.str.len);
  ...
  p = memchr(ret_span->ptr, '\\', (size_t)ret_span->len);
  if (!p) {
    return 0; // success
  }
  ...
  while (*p) {
    ...
    case 'x': {
      ...
      memcpy(buf, p + 2, 2);
      buf[2] = 0;
      int32_t ucs = (int32_t)strtol(buf, 0, 16);
  ...
  *dst = 0;
  ret_span->len = (int32_t)(dst - ret_span->ptr);
  return 0;
}
...
static int scan_multiline_string(scanner_t *sp, token_t *tok) {
  ...
  *tok = mktoken(sp, TOK_MLSTRING);
  ...
  tok->str.len = (int32_t)(sp->cur - tok->str.ptr);
...
static int scan_time(scanner_t *sp, token_t *tok) {
  ...
  char buffer[20];
  int len = (int)(sp->endp - sp->cur);
  if (len >= (int)sizeof(buffer)) {
    len = (int)sizeof(buffer) - 1;
  }
  memcpy(buffer, sp->cur, (size_t)len);
  buffer[len] = 0; // NUL
...
static int process_numstr(char *buffer, int base, const char **reason) {
  ...
  if (q) {
    for (int i = (int)(q - buffer); buffer[i]; i++) {
      if (buffer[i] != '_') {
        *q++ = buffer[i];
        continue;
      }
...
static int scan_literal(scanner_t *sp, token_t *tok) {
  *tok = mktoken(sp, TOK_LIT);
  ...
  tok->str.len = (int32_t)(p - tok->str.ptr);
  sp->cur = p;
  return 0;
}
...
static int utf8_to_ucs(const char *orig, int len, uint32_t *ret) {
  ...
  */
  if (0x6 == (i >> 5)) {
    ...
    return *ret = v, (int)(buf - (const unsigned char *)orig);
  }
