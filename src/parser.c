...
#define hhg_parser_msg(parser, type, ...)                                      \
    hhg_msg(parser->msg_ctx, type, &parser->lexer->src,                        \
            &parser->lexer->token.range, __VA_ARGS__)
...
static hhg_node_t *hhg_parser_parse_var_decl(hhg_parser_t *parser,
                                             hhg_type_t *type);
static hhg_node_t *hhg_parser_parse_obj_init(hhg_parser_t *parser,
                                             hhg_type_t *type);
...
static hhg_type_t *hhg_parser_parse_ref_type(hhg_parser_t *parser,
                                             hhg_type_t *base);
...
static hhg_node_t *hhg_parser_node_new(hhg_parser_t *parser,
...
{
    *parser = (hhg_parser_t){ .lexer = lexer,
                              .sym_tab = sym_tab,
                              .type_ctx = type_ctx,
                              .msg_ctx = msg_ctx,
                              .arena = arena };
...
hhg_node_t *hhg_parser_parse(hhg_parser_t *parser)
{
    ...
    prog->range = (hhg_file_range_t){ .start = start_pos,
...
static hhg_node_t *hhg_parser_parse_expr(hhg_parser_t *parser, int32_t min_prec)
{
    ...
    while (prec = parser->lexer->token.prec,
           prec != HHG_PREC_NONE && prec > min_prec) {
        hhg_node_t *new_left =
...
static hhg_node_t *hhg_parser_parse_unary(hhg_parser_t *parser)
{
    ...
    node->range = (hhg_file_range_t){ .start = start_pos,
...
static bool hhg_parser_is_type(hhg_parser_t *parser, hhg_token_t *token)
{
    if (token->type == HHG_TOKEN_ID) {
        hhg_sym_t *sym = hhg_sym_tab_lookup(parser->sym_tab, token->str.str);
        if (sym == NULL)
            return false;
...
static hhg_type_t *hhg_parser_parse_type(hhg_parser_t *parser)
{
    ...
    if ((cv_qual.is_const || cv_qual.is_volatile) && type != NULL) {
        type = hhg_type_ctx_new_cv_type(parser->type_ctx,
                                        (hhg_cv_tab_key_t){
                                            .base = type,
...
static hhg_type_t *hhg_parser_parse_base_type(hhg_parser_t *parser)
{
    ...
    if (parser->lexer->token.type == HHG_TOKEN_ID) {
        hhg_sym_t *sym =
            hhg_sym_tab_lookup(parser->sym_tab, parser->lexer->token.str.str);

        if (sym == NULL) {
            hhg_parser_error(parser, "unknown type `%s`", "`%s` used here",
        ...
        if (sym->value.sym_type != HHG_SYM_CLASS &&
            ...
            hhg_parser_error(parser, "`%s` must be a type", "`%s` used here",
                             parser->lexer->token.str.str);
            return NULL;
        }
        type = sym->value.type;
        ...
        if (base == HHG_TYPE_NONE) {
            hhg_parser_error(parser, "expected type, got `%s`", "here",
                             parser->lexer->token.str.str);
            return NULL;
        }

        type = hhg_type_ctx_get_builtin(parser->type_ctx, base);
    }
                                             ...
                                             hhg_type_t *base)
{
    while (parser->lexer->token.type == '&') {
        ...
        base = hhg_type_ctx_new_ref(parser->type_ctx,
                                    (hhg_ref_tab_key_t){
                                        .base = base,
                                        .is_const = cv_qual.is_const,
                                        .is_volatile = cv_qual.is_volatile,
                                             ...
                                             hhg_type_t *type)
{
    ...
    while (parser->lexer->token.type != ')' &&
        ...
        arrput(obj_init->value.obj_init.args,
               hhg_parser_parse_expr(parser, HHG_PREC_START));
...
static hhg_node_t *hhg_parser_parse_id(hhg_parser_t *parser)
{
    ...
    case '(': {
        hhg_node_t *func_call = hhg_parser_node_new(parser, HHG_NODE_FUNC_CALL);
        func_call->value.func_call.id.str = str;
        hhg_lexer_next(parser->lexer);
        while (parser->lexer->token.type != ')' &&
               parser->lexer->token.type != EOF) {
            arrput(func_call->value.func_call.args,
                   hhg_parser_parse_expr(parser, HHG_PREC_START));
            if (parser->lexer->token.type != ')')
                hhg_lexer_match(parser->lexer, ',');
        }
    ...
    case '.': {
        ...
        field_access->value.field_access.next = hhg_parser_parse_unary(parser);
...
static hhg_node_t *hhg_parser_parse_arr_literal(hhg_parser_t *parser)
{
    hhg_node_t *arr_literal = hhg_parser_node_new(parser, HHG_NODE_ARR_LITERAL);
    hhg_lexer_next(parser->lexer);
    while (parser->lexer->token.type != ']' &&
           parser->lexer->token.type != EOF) {
        arrput(arr_literal->value.arr_literal.elems,
               hhg_parser_parse_expr(parser, HHG_PREC_START));
        if (parser->lexer->token.type != ']')
            hhg_lexer_match(parser->lexer, ',');
    }
...
static hhg_node_t *hhg_parser_parse_if(hhg_parser_t *parser)
{
    ...
    if_stmt->value.if_stmt.cond = hhg_parser_parse_expr(parser, HHG_PREC_START);
...
static hhg_node_t *hhg_parser_parse_func_decl(hhg_parser_t *parser)
{
    ...
    while (parser->lexer->token.type != ')' &&
        ...
        param->range =
            (hhg_file_range_t){ .start = start_pos,
                                .end = parser->lexer->token.range.end };
...
static hhg_node_t *hhg_parser_parse_return(hhg_parser_t *parser)
{
    ...
    ret_stmt->value.ret.expr = hhg_parser_parse_expr(parser, HHG_PREC_START);
    return ret_stmt;
}
...
static hhg_node_t *hhg_parser_parse_class_decl(hhg_parser_t *parser)
{
    ...
    if (parser->lexer->token.type == HHG_TOKEN_ID) {
        ...
        hhg_sym_tab_insert(parser->sym_tab,
                           (hhg_sym_t){
    ...
    while (parser->lexer->token.type != '}' &&
        ...
        if (parser->lexer->token.type == HHG_TOKEN_DEF)
            ...
            if (var_decl->value_type == NULL)
                hhg_parser_error(parser,
                                 "expected type in class "
