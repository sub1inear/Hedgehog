...
void hhg_ext_build_init(hhg_ext_build_t *ext_build, const char *cxx,
                        hhg_msg_ctx_t *msg_ctx, hhg_arena_t *arena)
{
    if (cxx == NULL) {
        cxx = getenv("CXX");
        if (cxx == NULL)
            hhg_fatal_error("please specify a C++ compiler with --cxx or the "
                       ...
                       const char *out, bool release)
{
    ...
    if (exit_code != EXIT_SUCCESS)
        hhg_compiler_error("external build failed: `%s` exited with "
                           "code %i\noutput:\n%s",
                           ext_build->cxx, exit_code, stdouterr.str);
