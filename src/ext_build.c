...
#define hhg_ext_build_msg(ext_build, type, ...)                                \
    hhg_basic_msg(ext_build->msg_ctx, type, __VA_ARGS__)
...
static const char **hhg_ext_build_cl_copy_include(hhg_ext_build_t *ext_build,
                                                  const char **argv,
                                                  const char *cl_env);

static const char **hhg_ext_build_cl_copy_lib(hhg_ext_build_t *ext_build,
                                              const char **argv,
                                              const char *cl_env);

static const char **hhg_ext_build_cl_copy_core(
    hhg_ext_build_t *ext_build, const char **argv, const char *cl_env,
    const char *key,
    const char **(*copy_func)(hhg_ext_build_t *ext_build, const char **argv,
                              const char *loc, ptrdiff_t len));
static const char **
hhg_ext_build_cl_copy_include_core(hhg_ext_build_t *ext_build,
                                   const char **argv, const char *loc,
                                   ptrdiff_t len);
static const char **hhg_ext_build_cl_copy_lib_core(hhg_ext_build_t *ext_build,
                                                   const char **argv,
                                                   const char *loc,
                                                   ptrdiff_t len);
#endif
...
void hhg_ext_build_init(hhg_ext_build_t *ext_build, hhg_msg_ctx_t *msg_ctx,
                        hhg_arena_t *arena)
{
...
void hhg_ext_build_run(hhg_ext_build_t *ext_build, hhg_code_gen_t *code_gen,
                       const char *filename, const char *orig_filename,
                       const char *out, bool release)
{
    HHG_UNUSED(code_gen);

    const char *cxx = getenv("CXX");
    if (cxx == NULL)
        hhg_fatal_error("please specify a C++ compiler with --cxx or the CXX "
                        "environment variable");

    const char *cxx_name = hhg_arena_strdup(ext_build->arena, fs_basename(cxx));
                                           ...
                                           const char **argv)
{
    ...
    hhg_join_path(cl_env_cache_path, FS_MAX_PATH, HHG_TMP_DIR,
    ...
    if (fs_exist(cl_env_cache_path)) {
        ...
        if (ferror(cl_env_cache_file))
            hhg_fatal_error("%s: error reading file: %s", cl_env_cache_path,
                            strerror(errno));
        ...
        hhg_str_append_str(&vswhere_path,
        ...
        int vswhere_exit_code = hhg_spawn(
            (const char *[]){
                vswhere_path.str, "-latest", "-products", "*", "-requires",
                "Microsoft.VisualStudio.Component.VC.Tools.x86.x64",
                "-property", "installationPath", NULL},
            &vswhere_stdouterr);

        if (vswhere_exit_code != EXIT_SUCCESS)
            hhg_fatal_error("external build failed: `vswhere.exe` exited with "
                            "code %i\noutput:\n%s",
                            vswhere_exit_code, vswhere_stdouterr.str);

        while (vswhere_stdouterr.len > 0 &&
               isspace((unsigned char)
                           vswhere_stdouterr.str[vswhere_stdouterr.len - 1]))
            HHG_UNUSED(hhg_str_pop(&vswhere_stdouterr));

        if (vswhere_stdouterr.len == 0)
            hhg_fatal_error("external build failed: `vswhere.exe` did not "
                            "return a path to Visual Studio");
        ...
        if (vcvars_exit_code != EXIT_SUCCESS)
            hhg_fatal_error("external build failed: `vcvars64.bat` exited with "
                            "code %i\noutput:\n%s",
                            vcvars_exit_code, cl_env.str);
...
static const char **hhg_ext_build_cl_copy_include(hhg_ext_build_t *ext_build,
                                                  ...
                                                  const char *cl_env)
{
    return hhg_ext_build_cl_copy_core(ext_build, argv, cl_env, "INCLUDE",
...
{
    arrput(argv, "/link");
    argv = hhg_ext_build_cl_copy_core(ext_build, argv, cl_env, "LIB",
...
static const char **hhg_ext_build_cl_copy_lib_core(hhg_ext_build_t *ext_build,
                                                   ...
                                                   ptrdiff_t len)
{
    char *lib_path = hhg_arena_malloc(
        ext_build->arena, HHG_STR_LEN("/LIBPATH:") + len + 1 /* \0 */
    );
