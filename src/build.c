               ...
               hhg_arena_t *arena)
{
    ...
    hhg_build_check_exit_result_t sem_an_result =
        hhg_build_check_exit(build, msg_ctx,
                             &(hhg_build_stage_desc_t){
    ...
    if (sem_an_result == HHG_BUILD_CHECK_EXIT_ERROR)
        return false;
    else if (sem_an_result == HHG_BUILD_CHECK_EXIT_SAFE_EXIT)
        return true;
    ...
    hhg_build_check_exit_result_t mir_gen_result =
        hhg_build_check_exit(build, msg_ctx,
                             &(hhg_build_stage_desc_t){
    ...
    if (mir_gen_result == HHG_BUILD_CHECK_EXIT_ERROR)
        return false;
    else if (mir_gen_result == HHG_BUILD_CHECK_EXIT_SAFE_EXIT)
        return true;
    ...
    hhg_build_check_exit_result_t code_gen_result =
        hhg_build_check_exit(build, msg_ctx,
                             &(hhg_build_stage_desc_t){
    ...
    if (code_gen_result == HHG_BUILD_CHECK_EXIT_ERROR)
        return false;
    else if (code_gen_result == HHG_BUILD_CHECK_EXIT_SAFE_EXIT)
        return true;
    ...
    hhg_ext_build_run(&ext_build, &code_gen, code_gen_filename, build->entry,
    ...
    if (ext_build_result == HHG_BUILD_CHECK_EXIT_ERROR)
        return false;
    else if (ext_build_result == HHG_BUILD_CHECK_EXIT_SAFE_EXIT)
        return true;
...
{
    if (build_data->mir_gen)
        hhg_mir_gen_del(build_data->mir_gen);
    if (build_data->type_ctx)
        hhg_type_ctx_del(build_data->type_ctx);
    if (build_data->sym_tab)
        hhg_sym_tab_del(build_data->sym_tab);
    if (build_data->lexer)
        hhg_lexer_del(build_data->lexer);
    if (build_data->code_gen)
        hhg_code_gen_del(build_data->code_gen);
}
