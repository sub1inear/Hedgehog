...
void hhg_node_print(hhg_node_t *node, int32_t indent, bool use_sym)
{
    ...
    case HHG_TOKEN_DEF: {
        ...
        for (size_t i = 0; i < len; i++)
            hhg_node_print(node->value.func_decl.params[i], next_next_indent,
                           use_sym);
    ...
    case HHG_TOKEN_CLASS: {
        hhg_node_print_str("id", next_indent);
        hhg_node_print_id(node->value.class_decl.id, next_next_indent, use_sym);
        ...
        for (size_t i = 0; i < var_decls_len; i++)
            hhg_node_print(node->value.class_decl.var_decls[i],
                           next_next_indent,
        ...
        for (size_t i = 0; i < func_decls_len; i++)
            hhg_node_print(node->value.class_decl.func_decls[i],
                           next_next_indent,
