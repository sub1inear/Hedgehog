...
typedef enum hhg_mir_opnd_cnst_type {
    HHG_MIR_OPND_CNST_REG,
    HHG_MIR_OPND_CNST_LBL,
    HHG_MIR_OPND_CNST_CNST,
    HHG_MIR_OPND_CNST_SYM,
} hhg_mir_opnd_cnst_type_t;

typedef union hhg_mir_opnd_cnst_value {
    hhg_mir_reg_t reg;
    hhg_mir_lbl_t lbl;
    hhg_mir_cnst_t cnst;
    hhg_sym_t *sym;
} hhg_mir_opnd_cnst_value_t;

typedef struct hhg_mir_opnd_cnst {
    hhg_mir_opnd_cnst_type_t type;
    hhg_mir_opnd_cnst_value_t value;
} hhg_mir_opnd_cnst_t;
...
typedef struct hhg_mir_expr {
    hhg_mir_opnd_cnst_t left;
    hhg_mir_opnd_cnst_t right;
    hhg_mir_opnd_t dest;
} hhg_mir_expr_t;
...
typedef struct hhg_mir_call {
    hhg_sym_t *func;
    hhg_mir_opnd_cnst_t *args;
    hhg_mir_reg_t dest;
} hhg_mir_call_t;

typedef struct hhg_mir_func {
    hhg_sym_t *sym;
    hhg_mir_opnd_cnst_t *params;
} hhg_mir_func_t;

typedef struct hhg_mir_ret {
    hhg_mir_opnd_cnst_t value;
} hhg_mir_ret_t;
...
typedef struct hhg_mir_struct_init {
    hhg_sym_t *struct_type;
    hhg_mir_opnd_cnst_t *fields;
    hhg_mir_reg_t dest;
} hhg_mir_struct_init_t;

typedef struct hhg_mir_struct_load {
    hhg_mir_reg_t struct_ptr;
...
typedef struct hhg_mir_arr_init {
    ...
    hhg_mir_opnd_cnst_t *elems;
    hhg_mir_reg_t dest;
} hhg_mir_arr_init_t;

typedef struct hhg_mir_arr_load {
    hhg_mir_reg_t arr;
    hhg_mir_opnd_cnst_t idx;
    hhg_mir_reg_t dest;
} hhg_mir_arr_load_t;
